...
sont des coups gagants (any_t = 2941 pour any_t = 5230). En revanche, on voit apparaître des situations bloquantes (deadlock = 93). On s'apperçoit ici que le nombre 
...
cette fois le nombre de transitions any_t est supèrieur aux valeurs de dec21 et dec10 et la proportion de coups gagnants est plus faible (environ un sixième des 
transitions). 
...
situations redoutées (deadlock = SR). On à également le nombre de coup gagnants \textit{CCoupGagnant} qui augmente par rapport au nombre de transitions \textit{any_t}. 
