...
\textit{nbFailure} est une constante correspondant au nombre de vannes pouvant tomber en panne dans le système. Une vanne en panne est une vanne qui reste pour toujours dans son état actuel. Le système en possède trois et donc \textit{nbFailure} peut prendre les valeurs 0, 1, 2 ou 3. La contrainte \textit{nbFailure >= (V [0].fail + V [1].fail + V [2].fail)}  présente dans le système permet de s’assurer que la valeur de \textit{nbFailure} se situe entre 0 et 3 et qu’elle correspond au nombre de vannes en pannes.
