...
De plus, on peut voir qu'il y a 86 niveaux critiques et 86 situations redoutées, et environ un tier des transitions correspondent à un coup gagnant (CCoupGagnant = 1134 pour any_t = 3472).
...
au nombre de transition est faible (CCoupGagnant = 4950 pour any_t = 19450 soit environ un quart seulement) et un grand nombre d'états mènent à une situation où 
...
sont des coups gagants (any_t = 2941 pour any_t = 5230). En revanche, on voit apparaître des situations bloquantes (deadlock = 93). Il y a également moins 
...
effet d'augmenter les situation bloquantes (deadlock passe de 93 à 96) et de situations redoutées (SR passe de 93 à 96).
...
\paragraph{Interprétation des résultats}
% A COMPLETER
Avec une deuxième défaillance, on constate encore une fois une augmentation du nombre d'états (any_s = 1627) ce qui a pour conséquence de faire augmenter l'ensemble 
...
redoutées équivalent au nombre de niveaux critiques et des états out0, out1 et out2 en quantité presque similaires (un tier de chaque). En revanche, cette fois le 
nombre de transitions any_t est supèrieur aux valeurs de dec21 et dec10 et la proportion de coups gagnants est plus faible (environ un sixième des transitions).
